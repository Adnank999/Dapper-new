...
float scene(vec2 uv) {
  // Adjusting the positions of the circles to move them to the left
  float shiftAmount = -0.2; // Change this value to move more or less to the left

  // First circle's position
  vec2 pos1 = vec2(cos(time) * 0.4 + shiftAmount, sin(time * 2.0) * 0.2);
  float c1 = sdCircle(uv - pos1, 0.2);

  // Second circle's position
  vec2 pos2 = vec2(cos(time + 3.14) * 0.4 + shiftAmount, sin(time * 2.0 + 3.14) * 0.2);
  float c2 = sdCircle(uv - pos2, 0.16);
